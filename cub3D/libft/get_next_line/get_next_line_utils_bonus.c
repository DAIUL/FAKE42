...
char	*ft_strdup_gnlb(char *s)
{
	...
	if (!s || !*s)
		return (NULL);
	dup = malloc((ft_strlen_gnlb(s) + 1) * sizeof(char));
	if (!dup)
		return (NULL);
	while (s[i])
	{
...
char	*ft_strjoin_gnlb(char *s1, char *s2)
{
	...
	if (!s1)
		return (ft_strdup_gnlb(s2));
	s3 = malloc(ft_strlen_gnlb(s1) + ft_strlen_gnlb(s2) + 1 * sizeof(char));
	if (!s3)
		return (NULL);
	i = 0;
	while (s1[i])
	...
	while (s2[i])
	{
		s3[ft_strlen_gnlb(s1) + i] = s2[i];
		i++;
	}
	s3[ft_strlen_gnlb(s1) + i] = '\0';
	free(s1);
	return (s3);
}
